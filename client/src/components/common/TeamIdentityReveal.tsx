...
export const TeamIdentityReveal: React.FC<TeamIdentityRevealProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const t1 = setTimeout(() => setStep(1), 40);
      const t2 = setTimeout(() => setStep(2), 110);
      const t3 = setTimeout(() => setStep(3), 180);
      const t4 = setTimeout(() => setStep(4), 250);
      const t5 = setTimeout(() => setStep(5), 320);
      const t6 = setTimeout(() => setStep(6), 390);
      const t7 = setTimeout(() => setStep(7), 460);
  ...
  const displayTeam = teamName && teamName.trim().length > 0 ? teamName.trim() : 'ENGINEERING TEAM';
  ...
  const clipPathStyle = prefersReducedMotion
    ...
    : {
        ...
        transition: isClosing
          ? 'clip-path 200ms cubic-bezier(0.4, 0, 0.2, 1), opacity 180ms ease-out'
          : 'clip-path 400ms cubic-bezier(0.16, 1, 0.3, 1), opacity 220ms ease-out',
        willChange: 'clip-path, opacity',
      };

  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col justify-between bg-gradient-to-b from-[#f8fafc] via-[#f1f5f9] to-[#ffffff] text-slate-900 select-none overflow-y-auto overflow-x-hidden font-sans ${
        isClosing ? 'opacity-0 transition-opacity duration-200' : 'opacity-100 transition-opacity duration-220'
      }`}
      style={clipPathStyle}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <header
        className={`relative z-20 px-4 sm:px-8 md:px-12 pt-3.5 sm:pt-5 pb-1 flex items-center justify-between shrink-0 transition-all duration-300 ease-out ${
          step >= 1 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'
        }`}
      >
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 sm:px-6 md:px-8 py-3 sm:py-5 max-w-3xl mx-auto w-full my-auto">
        ...
        <div
          className={`transition-all duration-400 ease-out ${
            step >= 2 && !isClosing ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-2 scale-98'
          }`}
        >
        ...
        <div
          className={`w-24 sm:w-32 h-[1.5px] bg-gradient-to-r from-transparent via-[#0B4EA2]/40 to-transparent my-3 sm:my-3.5 transition-all duration-300 ease-out ${
            step >= 3 && !isClosing ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'
          }`}
        />
        ...
        <div
          className={`transition-all duration-300 ease-out ${
            step >= 3 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'
          }`}
        >
        ...
        <div
          className={`mt-2.5 sm:mt-3 max-w-full transition-all duration-400 ease-out ${
            step >= 4 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
          }`}
        >
        ...
        <div
          className={`mt-1.5 sm:mt-2 transition-all duration-300 ease-out ${
            step >= 4 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'
          }`}
        >
        ...
        <div
          className={`mt-2 sm:mt-2.5 transition-all duration-300 ease-out px-2 ${
            step >= 5 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'
          }`}
        >
        ...
        <div
          className={`w-full max-w-[360px] sm:max-w-[480px] md:max-w-[520px] mt-3.5 sm:mt-4.5 transition-all duration-500 ease-out ${
            step >= 6 && !isClosing ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-3 scale-98'
          }`}
        >
          {teamPhotoUrl ? (
        ...
        <div
          className={`mt-4 sm:mt-5 transition-all duration-400 ease-out ${
            step >= 7 && !isClosing ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
          }`}
        >
          <button
            type="button"
